...
def connect_to_database():
    return mysql.connector.connect(
        host="127.0.0.1",
        port=3306,
        database="flight_game3",  # database name here
        user="eku",  # user here # (this is crucial, add this line in your terminal: GRANT SELECT, INSERT, UPDATE ON databasename.* TO username@localhost;
...
def track_goals(player_data, chosen_country, chosen_continent):
    ...
    if chosen_continent == 'EU' and len(european_countries) >= 3:
        if "Visit European Countries" not in updated_goals:
            updated_goals.append("Visit European Countries")

    # Check goals for Asia (Fix: at least 3 unique Asian countries)
    if chosen_continent == 'AS' and len(asian_countries) >= 3:
        if "Visit Asian Countries" not in updated_goals:
            updated_goals.append("Visit Asian Countries")
...
def count_completed_goals(player_data):
    ...
    return len(completed_goals)  # Return the count of completed goals
...
def display_status(player_data):
    ...
    # Display rewards using the MEDAL_MAPPING
    reward = MEDAL_MAPPING[len(completed_goals)]
    print(f"Rewards: {Fore.GREEN}{reward}{Fore.RESET}")
